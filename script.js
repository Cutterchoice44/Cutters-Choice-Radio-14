...
function addMixcloud() {
  ...
  div.innerHTML = `<iframe src="https://www.mixcloud.com/widget/iframe/?hide_cover=1&light=1&feed=${URLSearchParams({url}).toString()}"></iframe><button class="delete-btn" onclick="deleteMixcloud(this)">Delete</button>`;
  ...
  fetch(`${BASE_URL}/station/${STATION_ID}/schedule/live`, {
    ...
    .then(data => {
      ...
      if (md?.artist && md?.title) {
        el.textContent = `Now Playing from Archives: ${md.artist} – ${md.title}`;
      } else if (ct?.title) {
        el.textContent = `Now Playing from Archives: ${ct.title}`;
      } else {
